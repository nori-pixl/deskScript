...
/**
 * ★根本バグ修正: これまで applyExtensions() は if/switch/while/try/forever/for/
 * lock:drawer/intern:desk/stamp/shift をそれぞれ「固定の順番」で、content文字列
 * 全体に対して個別にスキャンしていた。
 *
 * そのため、たとえば for(...){ if(...){...} } のように if が for の中に
 * ネストしていても、パイプライン内で IfBlock が ForLoopBlock より先に
 * 実行される設計だと、IfBlock が「まだ展開されていない for の中の if」を
 * 見つけて、外側のスコープ（forのループ変数が存在しないスコープ）で
 * 評価してしまい、"i is not defined" のような誤動作になっていた。
 *
 * この修正では、対象となる構文すべての「開始位置」を毎回スキャンし、
 * テキスト中で最も左（＝一番外側）にあるものから1つずつ処理する。
 * 選ばれた構文のbodyは ctx.runBody() 経由で再帰的にこの仕組みへ戻ってくるため、
 * どんな組み合わせ・深さのネストでも「外側から内側へ」正しい順番で処理される。
 */

interface DispatchResult {
  output: string;
  spanEnd: number; // out内で消費した最後の文字のインデックス（inclusive）
}
...
interface BlockDetector {
  regex: RegExp; // 非global。「構文の開始位置」だけを見つけるためのヘッダー正規表現
  processAt: ProcessAt;
}
...
function processIfAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
...
function processSwitchAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  const cases: { value: string; body: string }[] = [];
...
// --- while(条件):true{...} （安全のため最大反復回数あり） ---
function processWhileAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  ...
  while (count < MAX_WHILE_ITERATIONS) {
    const condResult = ctx.evaluator.evaluateExpression(m[1], hostScope, disScope);
    if (!(condResult === true || condResult === 'true')) break;
...
function processTryAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  const tryOpenIdx = m.index + m[0].length - 1;
  const tryCloseIdx = ctx.findMatchingBrace(out, tryOpenIdx);
  if (tryCloseIdx === -1) return null;
  ...
  if (catchCloseIdx === -1) return null;
  ...
  if (endBody) output += ctx.runBody(endBody, hostScope, disScope);
  return { output, spanEnd: cursor - 1 };
}

// --- forever(条件){...} （同期実行モデルでの安全な近似＝条件が真なら1回だけ実行） ---
function processForeverAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  const openIdx = m.index + m[0].length - 1;
  const closeIdx = ctx.findMatchingBrace(out, openIdx);
  if (closeIdx === -1) return null;
  const body = out.slice(openIdx + 1, closeIdx).trim();
  ...
  const condResult = ctx.evaluator.evaluateExpression(m[1], hostScope, disScope);
  const output = (condResult === true || condResult === 'true') ? ctx.runBody(body, hostScope, disScope) : '';
  return { output, spanEnd: closeIdx };
}

// --- for(dis.var:名前 in ++1):N{...} end{...} ---
function processForAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, _disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  const varName = m[1];
  const stepExpr = m[2];
  ...
  for (let l = 0; l < maxLoops; l++) {
    const iterDisScope: Record<string, any> = { [varName]: currentVal };
...
// --- lock:drawer(名前){...} ---
function processLockDrawerAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
...
function processInternDeskAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
...
function processStampAt(out: string, m: RegExpExecArray, hostScope: Record<string, any>, disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  const approvedOpenIdx = m.index + m[0].length - 1;
  const approvedCloseIdx = ctx.findMatchingBrace(out, approvedOpenIdx);
  if (approvedCloseIdx === -1) return null;
...
function processShiftAt(out: string, m: RegExpExecArray, _hostScope: Record<string, any>, _disScope: Record<string, any>, ctx: BlockContext): DispatchResult | null {
  let morningBody: string | null = null;
  let nightBody: string | null = null;
  let spanEnd: number;
...
const DETECTORS: BlockDetector[] = [
  ...
  { regex: /while\s*\(([^)]*)\)\s*:\s*true\s*\{/, processAt: processWhileAt },
  { regex: /try\s*\{/, processAt: processTryAt },
  { regex: /forever\s*\(([^)]*)\)\s*\{/, processAt: processForeverAt },
...
export function processNestableBlocks(
  content: string,
  hostScope: Record<string, any>,
  disScope: Record<string, any>,
  ctx: BlockContext
  ...
  while (safety++ < SAFETY_LIMIT) {
    let best: { index: number; match: RegExpExecArray; detector: BlockDetector } | null = null;
    ...
    if (!best) break; // これ以上、対象構文は見つからない
