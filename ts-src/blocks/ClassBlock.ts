...
export function processNewInstance(
  content: string,
  ...
  const regex = /new:(\w+)\(([^)]*)\)/g;
  return content.replace(regex, (_match, className: string, argsRaw: string) => {
    ...
    for (const f of classDef.fields) {
      if (!(f.name in fieldValues)) fieldValues[f.name] = '';
    }
    ...
    const selfScope: Record<string, any> = {};
    for (const key in fieldValues) selfScope[`self.${key}`] = fieldValues[key];
...
export function processInstanceFieldAccess(content: string, ctx: BlockContext): string {
  const regex = /(\w+#\d+)\.(\w+)/g;
  return content.replace(regex, (_match, instanceId: string, fieldName: string) => {
